// Apply database/027_whatsapp_cloud_followup.sql against the configured Postgres.
// Handles CREATE INDEX CONCURRENTLY correctly (runs each statement outside a tx).
// Usage: node scripts/apply-027.mjs "<connection-string>"
...
if (!conn) {
  console.error('Usage: node scripts/apply-027.mjs "<connection-string>"')
  process.exit(1)
}

const sqlFile = readFileSync(new URL('../database/027_whatsapp_cloud_followup.sql', import.meta.url), 'utf8')
